    ...
    <div className="flex flex-row-reverse items-center gap-3 h-64">
      ...
      <ul className="text-white">
        <li className="flex gap-2">
          {" "}
          <span className="w-10 h-5 bg-red-500 block"></span> Absent
        </li>
        <li className="flex gap-2">
          {" "}
          <span className="w-10 h-5 bg-green-500 block"></span> Present
        </li>
      </ul>
    </div>
  );
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center xl:pt-0 md:pt-40 pt-64 justify-center overflow-auto max-h-screen">
      ...
      <div className="flex gap-5 flex-wrap items-center justify-center">
        <>{graph}</>
        <div className="flow-root md:w-[400px] w-full bg-gray-900 px-7 py-5 rounded-lg shadow-xl max-h-[250px] overflow-auto">
          <span
          ...
          <ul role="list" className="divide-y divide-gray-700 text-white">
            {unmarkedStudents.length === 0
              ...
              : unmarkedStudents.map((student) =>
                  student.attendance === undefined ? (
                    <li
                      className="py-3 sm:py-4 px-5 rounded hover:bg-gray-800 hover:scale-105 transition-all"
                      key={student.id}
                    >
